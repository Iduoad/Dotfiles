...
c.url.start_pages = ['file:///home/guru/.config/browser/index.html']
...
config.bind('ybM', 'spawn echo {url} >> ~/Music/download.list')
config.bind('ybd', 'spawn st -e "youtube-dl {url} -o ~/Videos/%(title)s.%(ext)s"')
config.bind('ybm', 'spawn youtube-dl {url} -x --audio-format mp3 -o ~/Music/%(title)s.%(ext)s')
config.bind('ybp', 'spawn mpv --ytdl-format="18" {url}')
